...
2. WHEN 用户点击「新建定时任务」 THEN 系统 SHALL 弹出表单，依次采集：任务名(必填,≤40字)、Agent(必填)、版本策略、任务指令、调度方式；**不采集运行环境**（复用 Agent 发布设置，见下）
...
### Requirement: 运行环境复用 Agent 发布设置 🔸MVP
> 决策：运行环境（隔离级别）在**发布 Agent(I)** 时已经选过，定时任务不再单独配置——直接复用该 Agent 发布时的运行环境，避免同一个 Agent 出现「发布环境」与「定时环境」两处配置、语义打架。发布环境是唯一事实来源。

**User Story:** 作为项目成员，我希望定时任务直接沿用我发布该 Agent 时选的运行环境，这样只在一处配置环境、不必在建定时任务时重复选、也不会两处不一致。

#### Acceptance Criteria
1. WHERE 新建/编辑定时任务 系统 SHALL **不**提供运行环境选择控件；改为只读展示「跟随该 Agent 发布环境」+ 当前发布环境(如 L1/L2/L3)
2. WHEN 任务被触发 THEN 系统 SHALL 用该 Agent **发布时**的运行环境确保服务就绪（与发布保持同一环境）
3. IF 该 Agent 尚未发布(无发布环境) THEN 系统 SHALL 兜底用共享 L1，并在新建表单中明示「该 Agent 尚未发布，运行时默认用共享 L1」
...
- 现有功能：运行环境的选择归口到发布(I)；定时任务只消费不重复配置；语义/文案沿用 I 的 ISOLATIONS

#### 待确认 / 假设
- 已定：运行环境唯一来源是 Agent 发布设置；定时任务不再有独立 isolation（后端 isolation 列弃用）
