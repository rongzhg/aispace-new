...
version: 2
description: Chat 统一入口——默认通用 agent，slash 唤起内置 skill；目标：平台所有功能都能通过 chat 解决
---
...
侧边栏「**Chat**」是平台统一入口（copilot）：界面能做的 chat 都能做，chat 还能理解需求。一个对话线程，默认通用 agent；输入 `/` 唤起内置 skill。
...
### Requirement: Chat 入口（单线程 + slash） 🔸MVP（已在 demo 实现，阶段一）
**User Story:** 作为用户，我希望有一个统一聊天入口，默认就能对话/操作，并能用 / 选择内置技能。

#### Acceptance Criteria
1. WHERE 侧边栏 系统 SHALL 提供「Chat」入口，与 Agents/Playground/成员并列
2. WHEN 用户进入 Chat THEN 系统 SHALL 默认进入**通用 agent**单线程对话并展示用法引导
3. WHEN 用户输入 `/` THEN 系统 SHALL 弹出内置 skill 菜单（`/agent-creator`、`/skill-creator`，按输入过滤），点击或输入命令即进入该 skill
4. WHERE 处于某 skill 系统 SHALL 显示当前技能标识并提供「× 退出」回到通用 agent
5. WHERE skill 菜单由注册表驱动 系统 SHALL 支持后续扩展（内置/用户 skill 自动出现）

#### 引用 / 影响
- 组件：单线程对话面板 + slash 下拉菜单 + 当前技能 chip
- 现有功能：复用对话气泡；通用 agent 执行见下；与 Skill 概念(E)对齐
- 设计决策：去掉三选一切换器，改为单线程 + slash（更像 copilot）
...
#### Acceptance Criteria
1. WHERE 每个用户 系统 SHALL 提供一个常驻「通用助手」（copilot），跑在该用户的 L0 沙箱里、**跨其所有空间复用**；按**用户身份**路由（一条 `/api/copilot/chat` 对所有用户），「当前空间」作每请求上下文（见 架构设计 §10.5）
...
#### 待确认 / 假设
- ⬜后续：多 session（每对话一线程 + 落库），承载异步任务与可追溯
