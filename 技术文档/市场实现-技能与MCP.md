...
installed_mcp   (ws_id, id, name, summary, category, command, args(json), env(json),
                 homepage, source, added_at,  PK(ws_id,id))
```
...
| GET | `/api/market/mcp?q=&category=` | MCP 市场目录（全平台共享）；`{items, categories}` |
| POST | `/api/market/mcp/{id}/install?ws=` | 把目录项加入当前空间 |
| POST | `/api/market/mcp/register?ws=` | 注册自定义 MCP 到当前空间（body: name/desc/category/command/args/env/homepage） |
| GET | `/api/tools?ws=` | 某空间已启用 MCP 工具（供该空间 Agent 工具选择） |
| DELETE | `/api/tools/{id}?ws=` | 从当前空间移出 |
...
- `components.tsx`：`Market({wsId, wsName})`（Tabs：MCP 市场 / 技能市场，顶部标注当前空间）+ `McpMarket` / `SkillMarket` / `MarketCard` / `EnabledStrip`。
  ...
  - **MCP 市场**：与技能市场对称——顶部「注册 MCP」按钮 → Modal 表单（参数按行拆、环境变量按逗号/换行拆）；已注册 MCP 以 `MarketCard`（`installed`）卡片网格展示，无内置目录浏览/搜索/分类。
  - MCP 卡片点「详情」→ `McpDetailModal`：前端按 command/args/env 拼出**标准 `mcpServers` 配置**（`type:stdio`）展示在浅色 `<pre>` + 复制按钮，附分类/来源/主页/凭证；不导出 `.mcp.json`（仅展示）。
  - `AssetDrawer`（Agent 工具/技能选择）两侧都只用后端已注册项，不再混入内置 mock `TOOLS`/`SKILLS`。
  - **视觉规范对齐**（产品文档/视觉规范.dc.html）：卡片/面板圆角 8px（不用 ≥12px）、描边 `#EBEBF1`、警告类药丸用 `#FFF8E6/#946C00`、代码/预览用浅色面（`#FAFAFC` + `#F1F1F5` 边），主按钮走主题墨色 `#17171C`。
...
curl 'localhost:8000/api/market/mcp' --data-urlencode 'category=数据库' -G       # -> postgres, sqlite
curl 'localhost:8000/api/market/skills?q=postgres&limit=3'                        # -> 带 score 的技能
curl -X POST 'localhost:8000/api/market/mcp/github/install?ws=w1'                 # 加入 w1
curl -X POST 'localhost:8000/api/market/mcp/register?ws=w1' \
  -H 'Content-Type: application/json' \
  -d '{"name":"内部工单系统","command":"node","args":["/srv/x.js"],"env":["JIRA_TOKEN"]}'  # 注册自定义
curl 'localhost:8000/api/tools?ws=w1'   # w1 已启用（github + 自定义）
curl 'localhost:8000/api/tools?ws=w2'   # [] —— 空间隔离
# 前端：npm run build 通过；dev server 经 /api 代理到 8000
```

## 后续（见 spec ⬜）
- 技能「加入平台」当前只登记进可选目录；真正把技能实体拉取/安装到运行环境待做（与「技能注册/上传」打通）。
- MCP「加入平台」未校验连通性、未托管凭证（token）；把已装 MCP 导出成 Agent 工作目录的 `.mcp.json` 也待做（格式见 `templates/general-agent/mcp.json`）。
